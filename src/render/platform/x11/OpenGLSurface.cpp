...
struct GlxSurfaceBackend::Impl  {
    ...
    Base::Result<void> Create(
        ...
        if (candidate.kind != SurfaceKind::GlxWindow ||
...
GlxSurfaceBackend::GlxSurfaceBackend(
    Base::IAllocator* allocator) noexcept
    : allocator_(allocator != nullptr
          ? allocator
          : &Base::GetDefaultAllocator()) {}

GlxSurfaceBackend::~GlxSurfaceBackend() noexcept {
    DestroySurface();
}

std::uintptr_t GlxSurfaceBackend::NativeDisplay() const noexcept {
    return impl_ != nullptr
        ? reinterpret_cast<std::uintptr_t>(impl_->display)
        : 0U;
}

std::uintptr_t GlxSurfaceBackend::NativeDrawable() const noexcept {
    return impl_ != nullptr
        ? static_cast<std::uintptr_t>(impl_->drawable)
        : 0U;
}

std::uintptr_t GlxSurfaceBackend::NativeContext() const noexcept {
    return impl_ != nullptr
        ? reinterpret_cast<std::uintptr_t>(impl_->context)
        : 0U;
}

bool GlxSurfaceBackend::OwnsDisplay() const noexcept {
    return impl_ != nullptr && impl_->ownsDisplay;
}

bool GlxSurfaceBackend::OwnsDrawable() const noexcept {
    return impl_ != nullptr && impl_->ownsDrawable;
}

bool GlxSurfaceBackend::OwnsContext() const noexcept {
    return impl_ != nullptr && impl_->ownsContext;
}

GlContextGeneration
GlxSurfaceBackend::ContextGeneration() const noexcept {
    return impl_ != nullptr ? impl_->generation : 0U;
}

Base::Result<GlFunctionTable>
GlxSurfaceBackend::LoadFunctions() noexcept {
    if (impl_ == nullptr) {
        return NotInitialized(
            "GLX surface is not initialized");
    }
...
Base::Result<void> GlxSurfaceBackend::MakeCurrent() noexcept {
    return impl_ != nullptr
...
SurfaceCapabilities
GlxSurfaceBackend::QuerySurfaceCapabilities() const noexcept {
...
Base::Result<void> GlxSurfaceBackend::CreateSurface(
...
void GlxSurfaceBackend::DestroySurface() noexcept {
    if (impl_ == nullptr) {
        return;
    }
    impl_->Reset();
...
Base::Result<void> GlxSurfaceBackend::ResizeSurface(
    std::uint32_t width,
    std::uint32_t height) noexcept {
    if (impl_ == nullptr) {
        return NotInitialized(
...
GlxSurfaceBackend::AcquireSurfaceTarget(
...
Base::Result<void> GlxSurfaceBackend::PresentSurface(
...
void GlxSurfaceBackend::DiscardSurfaceFrame(
    std::uint64_t frameSerial) noexcept {
...
void GlxSurfaceBackend::NotifySurfaceLost() noexcept {
    if (impl_ != nullptr) {
        impl_->ReleaseActiveFrame();
        impl_->lost = true;
    }
...
Base::Result<void> GlxSurfaceBackend::RestoreSurface(
    const NativeSurfaceDescriptor& descriptor) noexcept {
...
bool GlxSurfaceBackend::IsSurfaceLost() const noexcept {
