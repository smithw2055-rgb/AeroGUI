...
#if !defined(_WIN32)
#error "OpenGLRenderContext.cpp is only supported on Windows"
#endif
...
struct WglRenderContext::Impl  {
    ...
    Base::Result<void> Create(
        ...
        if (candidate.kind != WindowRenderContextKind::Wgl ||
...
WglRenderContext::~WglRenderContext() noexcept {
    Shutdown();
}

std::uintptr_t
WglRenderContext::NativeDeviceContext() const noexcept {
...
std::uintptr_t
WglRenderContext::NativeRenderContext() const noexcept {
    return impl_ != nullptr
        ? reinterpret_cast<std::uintptr_t>(impl_->renderContext)
        : 0U;
}

bool WglRenderContext::OwnsContext() const noexcept {
    return impl_ != nullptr && impl_->ownsContext;
}

GlContextGeneration
WglRenderContext::ContextGeneration() const noexcept {
    return impl_ != nullptr ? impl_->generation : 0U;
}

Base::Result<GlFunctionTable>
WglRenderContext::LoadFunctions() noexcept {
    if (impl_ == nullptr) {
        return NotInitialized(
            "WGL surface is not initialized");
    }
...
WglRenderContext::ContextBinding() noexcept {
    if (impl_ == nullptr) {
        return NotInitialized(
            "WGL surface is not initialized");
    }
...
Base::Result<void> WglRenderContext::MakeCurrent() noexcept {
    return impl_ != nullptr
...
WglRenderContext::Caps() const noexcept {
    WindowRenderContextCaps capabilities;
    capabilities.supportedKinds =
        WindowRenderContextKindBit(WindowRenderContextKind::Wgl);
...
Base::Result<void> WglRenderContext::Create(
...
void WglRenderContext::Shutdown() noexcept {
    if (impl_ == nullptr) {
        return;
    }
    impl_->Reset();
...
Base::Result<void> WglRenderContext::Resize(
    std::uint32_t width,
...
WglRenderContext::AcquireTarget(
...
Base::Result<void> WglRenderContext::Present(
...
void WglRenderContext::DiscardFrame(
    std::uint64_t frameSerial) noexcept {
...
void WglRenderContext::NotifyLost() noexcept {
    if (impl_ != nullptr) {
        impl_->ReleaseActiveFrame();
        impl_->lost = true;
    }
...
Base::Result<void> WglRenderContext::Restore(
    const WindowRenderContextDescriptor& descriptor) noexcept {
...
bool WglRenderContext::IsLost() const noexcept {
