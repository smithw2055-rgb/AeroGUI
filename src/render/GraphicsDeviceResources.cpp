...
Base::Result<void> GraphicsDevice::Initialize() noexcept {
    ...
    if (capabilities_.abiVersion != GraphicsDeviceAbiVersion ||
...
Base::Result<void> GraphicsDevice::VerifyReady() const noexcept {
    if (!initialized_) {
        return Base::Status::Failure(
...
Base::Result<ResourceHandle> GraphicsDevice::CreateResource(
...
void GraphicsDevice::Rollback(ResourceHandle handle) noexcept {
...
Base::Result<ResourceHandle> GraphicsDevice::CreateBuffer(
...
Base::Result<ResourceHandle> GraphicsDevice::CreateTexture(
...
Base::Result<ResourceHandle> GraphicsDevice::CreateExternalTexture(
    const TextureResourceDescriptor& descriptor) noexcept {
    Base::Result<void> valid = ValidateTextureDescriptor(
        descriptor, backend_->QueryGraphicsCapabilities());
    if (!valid) return valid.GetStatus();
...
Base::Result<ResourceHandle> GraphicsDevice::CreateRenderTarget(
    const TextureResourceDescriptor& descriptor) noexcept {
    Base::Result<void> valid = ValidateTextureDescriptor(
        descriptor, backend_->QueryGraphicsCapabilities());
    if (!valid) return valid.GetStatus();
...
Base::Result<ResourceHandle> GraphicsDevice::CreateExternalRenderTarget(
    const TextureResourceDescriptor& descriptor) noexcept {
    Base::Result<void> valid = ValidateTextureDescriptor(
        descriptor, backend_->QueryGraphicsCapabilities());
    if (!valid) return valid.GetStatus();
...
Base::Result<ResourceHandle> GraphicsDevice::CreateSampler(
...
Base::Result<ResourceHandle> GraphicsDevice::CreatePipeline(
...
bool GraphicsDevice::IsAlive(ResourceHandle handle) const noexcept {
...
Base::Result<void> GraphicsDevice::DestroyResource(
...
Base::Result<FenceValue> GraphicsDevice::Submit(
...
Base::Result<std::uint32_t> GraphicsDevice::CollectGarbage() noexcept {
    Base::Result<void> ready = VerifyReady();
    if (!ready) return ready.GetStatus();
...
std::uint32_t GraphicsDevice::LiveResourceCount() const noexcept {
