...
static_assert(sizeof(ShaderRectConstants) % 16U == 0U,
    "Renderer constants must be float4 aligned");
static_assert(sizeof(ShaderRectConstants) <= 64U * 1024U,
    "Renderer constants must not exceed 64 KiB");
...
struct Renderer::Impl final {
    Impl(
        ...
        : device(&device), nodes(allocator), transforms(allocator),
...
Base::Result<void> Renderer::Initialize() noexcept {
    ...
    if (device_ == nullptr || device_->IsDeviceLost()) {
        return NotInitialized("Renderer requires a ready graphics device");
    }
    if (shaders_.colorFormat == GraphicsTextureFormat::Depth24Stencil8 ||
        shaders_.sampleCount == 0U) {
        return Unsupported("Renderer shader set or graphics capabilities are invalid");
    }
    if (impl_ == nullptr) {
    ...
    pipelineDescriptor.vertexShader = shaders_.rectangle.vertex;
    pipelineDescriptor.fragmentShader = shaders_.rectangle.fragment;
    ...
    pipelineDescriptor.sampleCount = shaders_.sampleCount;
    pipelineDescriptor.raster.scissorEnabled = true;
    Base::Result<ResourceHandle> pipeline = device_->CreatePipeline(pipelineDescriptor);
    if (!pipeline) {
        Shutdown();
    ...
    imagePipelineDescriptor.vertexShader = shaders_.image.vertex;
    imagePipelineDescriptor.fragmentShader = shaders_.image.fragment;
    ...
    meshPipelineDescriptor.vertexShader = shaders_.mesh.vertex;
    meshPipelineDescriptor.fragmentShader = shaders_.mesh.fragment;
    ...
    glyphPipelineDescriptor.vertexShader = shaders_.glyph.vertex;
    glyphPipelineDescriptor.fragmentShader = shaders_.glyph.fragment;
...
const RendererStatistics& Renderer::LastStatistics() const noexcept {
    static const RendererStatistics Empty;
    return impl_ != nullptr ? impl_->lastStatistics : Empty;
}

Base::Result<GraphicsCommandBuffer> Renderer::Record(
    ...
    if (!target.color.IsValid() ||
        ...
        target.format != shaders_.colorFormat ||
        target.sampleCount != shaders_.sampleCount) {
        return InvalidArgument("Renderer target is invalid or incompatible with its pipelines");
    }
