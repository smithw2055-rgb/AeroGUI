...
struct D3D11RenderDeviceState  {
    ...
    explicit D3D11RenderDeviceState(
    ...
    ~D3D11RenderDeviceState() noexcept {
        Reset();
    }
...
static_assert(
    sizeof(D3D11RenderDeviceState) <= 16384,
    "D3D11 render-device inline state storage is too small");
static_assert(
    alignof(D3D11RenderDeviceState) <= alignof(std::max_align_t),
    "D3D11 render-device state requires stronger alignment");
