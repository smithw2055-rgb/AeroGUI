...
Base::Result<GraphicsBackend*> SelectGraphicsBackend(
    Base::Span<GraphicsBackend*> backends,
    const BackendRequest& request) noexcept {
    GraphicsBackend* fallback = nullptr;
    for (GraphicsBackend* backend : backends) {
        ...
        if (device.abiVersion != GraphicsDeviceAbiVersion ||
...
DeviceCapabilities NullGraphicsBackend::Capabilities() const noexcept {
    ...
    capabilities.abiVersion = GraphicsDeviceAbiVersion;
