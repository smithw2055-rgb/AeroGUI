...
struct TextBlockRenderService::Impl final {
...
Base::Result<void> TextBlockRenderService::Initialize(
    const TextBlockRenderServiceConfig& config) noexcept {
    if (impl_ != nullptr) {
...
void TextBlockRenderService::Shutdown() noexcept {
    ...
    const Rhi::FenceValue retireFence =
        device_ != nullptr ? device_->LastSubmittedFence() : 0U;
...
bool TextBlockRenderService::IsInitialized() const noexcept {
    return impl_ != nullptr && impl_->initialized;
}

Base::Result<std::uint32_t>
TextBlockRenderService::CollectGarbage() noexcept {
    if (!IsInitialized()) {
        return Base::Status::Failure(
            Base::ErrorCode::NotInitialized,
            "TextBlock render service is not initialized");
    ...
    while (index < impl_->runs.Size()) {
        ...
        Base::Result<void> unregistered =
            registry_->UnregisterGlyphRun(run.id);
        if (!unregistered &&
...
Base::Result<void> TextBlockRenderService::ShapeAndPrepare(
    const Controls::TextBlockLayoutRequest& request,
    Controls::TextBlockLayoutResult& output) noexcept {
    if (!IsInitialized()) {
        return Base::Status::Failure(
            Base::ErrorCode::NotInitialized,
            "TextBlock render service is not initialized");
    ...
    for (BatchBuild& batch : batches) {
        if (impl_->nextGlyphRun ==
            ...
            for (std::uint32_t rollback = 0U;
                ...
                (void)registry_->UnregisterGlyphRun(
        ...
        if (!appended) {
            (void)registry_->UnregisterGlyphRun(id);
            for (std::uint32_t rollback = 0U;
                 rollback < registeredCount; ++rollback) {
                (void)registry_->UnregisterGlyphRun(
                    prepared[rollback].id);
...
void TextBlockRenderService::ReleaseGlyphRun(
...
TextBlockRenderServiceStatistics
TextBlockRenderService::Statistics() const noexcept {
    TextBlockRenderServiceStatistics statistics;
    if (!IsInitialized()) return statistics;
    statistics.atlasPages = impl_->atlas.PageCount();
    statistics.atlasEntries = impl_->atlas.EntryCount();
    statistics.lastUploadFence =
        impl_->lastUploadFence;
    for (const Impl::RunResource& run : impl_->runs) {
        if (run.released) {
            ++statistics.retiredGlyphRuns;
        } else {
            ++statistics.activeGlyphRuns;
        }
    }
    return statistics;
}
