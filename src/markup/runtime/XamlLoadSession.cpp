...
class XamlNodeCursor {
public:
    virtual ~XamlNodeCursor() = default;
    virtual Base::Result<const XamlNode*> Read(
        XamlNode& scratch) noexcept = 0;
};

namespace {

class StreamingXamlNodeCursor final : public XamlNodeCursor {
public:
    explicit StreamingXamlNodeCursor(
        XamlNodeReader& reader) noexcept
        : reader_(&reader) {}

    Base::Result<const XamlNode*> Read(
        XamlNode& scratch) noexcept override {
        Base::Result<XamlNodeKind> read =
            reader_->Read(scratch);
        return read
            ? Base::Result<const XamlNode*>(&scratch)
            : Base::Result<const XamlNode*>(
                  read.GetStatus());
    }

private:
    XamlNodeReader* reader_ = nullptr;
};

class CompiledXamlNodeCursor final : public XamlNodeCursor {
public:
...
private:
    Base::Span<const XamlNode> nodes_;
...
XamlLoadSession::XamlLoadSession(
...
XamlLoadSession::~XamlLoadSession() noexcept {
    AbortTransaction();
}

Base::Result<XamlLoadResult> XamlLoadSession::Load(
    XamlNodeReader& reader) noexcept {
    ...
    Base::Result<XamlLoadResult> result =
...
Base::Result<XamlLoadResult> XamlLoadSession::CompleteLoad(
...
Base::Result<Base::Ref<Base::Object>> XamlLoadSession::LoadReaderCore(
    XamlNodeReader& reader) noexcept {
    StreamingXamlNodeCursor cursor(reader);
    return LoadCursorCore(cursor);
}

Base::Result<Base::Ref<Base::Object>> XamlLoadSession::LoadCursorCore(
    ...
    XamlNode node;
    while (!ended_) {
        Base::Result<const XamlNode*> readResult =
            cursor.Read(node);
        if (!readResult) {
        ...
        const XamlNode* current = readResult.Value();
        if (current == nullptr) {
...
Base::Result<Base::Ref<Base::Object>> XamlLoadSession::LoadCompiledCore(
    const XamlCompiledDocument& document) noexcept {
    if (consumed_ || loading_) return SessionConsumedStatus();
...
Base::Result<Base::Ref<Base::Object>> XamlLoadSession::CreateObject(
    ...
    if (loadContext_ != nullptr &&
        ...
        if (actual == Core::InvalidTypeId ||
            !schema_->Descriptors().IsAssignableFrom(type, actual)) {
            return Base::Status::Failure(
    ...
    if (loadContext_ != nullptr &&
        loadContext_->activationFacets != nullptr) {
        if (loadContext_->activation == nullptr) {
            return Base::Status::Failure(
                Base::ErrorCode::InvalidState,
                "XAML load context has activation facets but no activation data");
        }
        if (loadContext_->activationFacets->Find(type) != nullptr) {
            return loadContext_->activationFacets->CreateObject(
...
Base::Result<void> XamlLoadSession::ProcessNode(
...
Base::Result<void> XamlLoadSession::QueueNamespaceDeclaration(
    const XamlNode& node) noexcept {
...
Base::Result<void> XamlLoadSession::StartObject(
    const XamlNode& node) noexcept {
...
Base::Result<void> XamlLoadSession::StartValueObject(
    const XamlNode& node,
...
Base::Result<void> XamlLoadSession::StartNullObject(
    const XamlNode& node,
...
Base::Result<void> XamlLoadSession::EndObject(
    const XamlNode& node) noexcept {
    if (frames_.Empty()) {
        return Failure(
            InvalidStateStatus(),
...
Base::Result<void> XamlLoadSession::StartMember(
    const XamlNode& node) noexcept {
    if (frames_.Empty() ||
        (frames_.Back().kind != FrameKind::Object &&
...
Base::Result<void> XamlLoadSession::StartDirective(
    const XamlNode& node,
...
Base::Result<void> XamlLoadSession::EndMember(
    const XamlNode& node) noexcept {
    if (frames_.Empty()) {
        return Failure(
            InvalidStateStatus(),
...
Base::Result<void> XamlLoadSession::WriteText(
    const XamlNode& node) noexcept {
    ...
    if (frame.kind == FrameKind::Member) {
        ...
        if (markup == MarkupValueKind::Extension) {
            Base::Result<XamlProvidedValue> value =
...
Base::Result<void> XamlLoadSession::WriteDirectiveText(
    Frame& frame,
    const XamlNode& node) noexcept {
...
Base::Result<void> XamlLoadSession::StartPropertyElement(
    const XamlNode& node,
...
Base::Result<void> XamlLoadSession::CompleteObject(
    const XamlNode& node) noexcept {
...
Base::Result<void> XamlLoadSession::CompleteValueObject(
    const XamlNode& node) noexcept {
    if (frames_.Empty() ||
        frames_.Back().kind != FrameKind::ValueObject) {
        return Failure(
...
Base::Result<void> XamlLoadSession::CompleteNullObject(
    const XamlNode& node) noexcept {
...
Base::Result<void> XamlLoadSession::WriteValueToParent(
    XamlValue&& value,
...
Base::Result<void> XamlLoadSession::WriteObjectToParent(
...
Base::Result<void> XamlLoadSession::WriteObjectToContent(
...
Base::Result<void> XamlLoadSession::WriteNullToParent(
    Core::SourceSpan source) noexcept {
    if (frames_.Empty()) {
        return Failure(
            Base::Status::Failure(
...
Base::Result<void> XamlLoadSession::WriteValueToMember(
    Frame& memberFrame,
    XamlValue&& value,
    Core::SourceSpan source) noexcept {
...
Base::Result<void> XamlLoadSession::WriteProvidedValueToMember(
    Frame& memberFrame,
    XamlProvidedValue&& provided,
    Core::SourceSpan source) noexcept {
...
Base::Result<void> XamlLoadSession::WriteProvidedValue(
    std::uint32_t targetObjectIndex,
    const XamlResolvedMember& member,
    XamlProvidedValue&& provided,
    Core::SourceSpan source) noexcept {
    if (provided.kind == XamlProvidedValueKind::Value) {
        return WriteValue(
            targetObjectIndex,
            member,
...
Base::Result<void> XamlLoadSession::WriteValue(
    std::uint32_t targetObjectIndex,
    const XamlResolvedMember& member,
    XamlValue&& value,
    Core::SourceSpan source) noexcept {
...
Base::Result<void> XamlLoadSession::RegisterObjectName(
    std::uint32_t objectIndex,
    Core::SourceSpan source) noexcept {
    if (objectIndex >= created_.Size()) {
        return Failure(
...
Base::Result<bool> XamlLoadSession::RegisterObjectResource(
    std::uint32_t objectIndex,
    Core::SourceSpan source) noexcept {
    if (objectIndex >= created_.Size()) {
        return Failure(
...
Base::Result<XamlResourceValue> XamlLoadSession::LookupResource(
...
Base::Result<void> XamlLoadSession::CreateScopesForObject(
    std::uint32_t objectIndex,
    Frame& frame,
    Core::SourceSpan source) noexcept {
    if (objectIndex >= created_.Size()) {
...
Base::Result<void> XamlLoadSession::ActivatePendingNamespaces(
    std::uint32_t& bindingStart) noexcept {
...
void XamlLoadSession::PopNamespaceBindings(
    std::uint32_t bindingStart) noexcept {
...
Base::Result<Base::StringView> XamlLoadSession::LookupNamespace(
...
XamlServiceProvider XamlLoadSession::BuildServices(
    std::uint32_t targetObjectIndex,
    const XamlResolvedMember& member,
    Core::SourceSpan source) noexcept {
    ...
    services.namespaces = XamlNamespaceScope(
        &XamlLoadSession::NamespaceLookupCallback,
        this);
    services.resources = XamlResourceResolver(
        &XamlLoadSession::ResourceLookupCallback,
        this);
    ...
    for (std::uint32_t index = frames_.Size();
        ...
        for (const ResourceDictionary* existing :
             serviceResourceChain_) {
...
const NameScope* XamlLoadSession::FindActiveNameScope() const noexcept {
    for (std::uint32_t index = frames_.Size(); index > 0U; --index) {
        const Frame& frame = frames_[index - 1U];
        if (frame.kind == FrameKind::Object &&
...
Base::Object*
XamlLoadSession::FindDeferredContentOwner() const noexcept {
    for (std::uint32_t index = frames_.Size();
         index > 0U;
         --index) {
        const Frame& frame = frames_[index - 1U];
...
std::uint32_t XamlLoadSession::FindNameScopeIndexForObject(
...
std::uint32_t XamlLoadSession::FindResourceScopeIndexForParent() const noexcept {
    for (std::uint32_t index = frames_.Size(); index > 0U; --index) {
        const Frame& frame = frames_[index - 1U];
        if (frame.kind == FrameKind::Object &&
            frame.resourceScopeIndex != InvalidIndex) {
...
std::uint32_t XamlLoadSession::FindObjectFrameIndex(
    std::uint32_t objectIndex) const noexcept {
    for (std::uint32_t index = frames_.Size(); index > 0U; --index) {
        const Frame& frame = frames_[index - 1U];
        if (frame.kind == FrameKind::Object &&
...
XamlLoadSession::MarkupValueKind XamlLoadSession::ParseMarkupValue(
...
Base::Result<XamlProvidedValue> XamlLoadSession::EvaluateMarkupExtension(
    std::uint32_t targetObjectIndex,
    const XamlResolvedMember& member,
    Base::StringView extensionName,
    ...
    const XamlServiceProvider services = BuildServices(
        ...
        member,
        source);
    Base::Result<XamlProvidedValue> provided =
...
bool XamlLoadSession::IsXamlDirective(
...
bool XamlLoadSession::IsWhitespaceOnly(
...
XamlLoadSession::AssignmentRecord* XamlLoadSession::FindAssignment(
    std::uint32_t objectIndex,
...
void XamlLoadSession::CommitDocumentScopes() noexcept {
    committedNames_.Clear();
    committedResources_.Clear();
...
void XamlLoadSession::AbortTransaction() noexcept {
    root_.Reset();
    resultVisualContent_.ReleaseContent();
    resultVisualContent_.Clear();
...
void XamlLoadSession::ClearTransaction() noexcept {
    deferredContent_.ReleaseAll();
    frames_.Clear();
    assignments_.Clear();
    extensionEffects_.Clear();
...
Base::Status XamlLoadSession::Failure(
    Base::Status status,
...
Base::Result<Base::StringView> XamlLoadSession::NamespaceLookupCallback(
    void* context,
    ...
    return static_cast<XamlLoadSession*>(context)->LookupNamespace(prefix);
}

Base::Result<XamlResourceValue> XamlLoadSession::ResourceLookupCallback(
    void* context,
    ...
    return static_cast<XamlLoadSession*>(context)->LookupResource(key);
}
