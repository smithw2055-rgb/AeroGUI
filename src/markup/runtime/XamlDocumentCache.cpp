...
struct XamlDependencyGraph::Impl final {
    struct Node final {
...
XamlDependencyGraph::XamlDependencyGraph(
    Base::IAllocator* allocator) noexcept
...
XamlDependencyGraph::~XamlDependencyGraph() noexcept {
...
XamlDependencyGraph::XamlDependencyGraph(
    XamlDependencyGraph&& other) noexcept
    : allocator_(other.allocator_), impl_(other.impl_) {
    other.allocator_ = nullptr;
    other.impl_ = nullptr;
}

XamlDependencyGraph& XamlDependencyGraph::operator=(
    XamlDependencyGraph&& other) noexcept {
...
Base::Result<void> XamlDependencyGraph::Update(
...
bool XamlDependencyGraph::Remove(
...
void XamlDependencyGraph::Clear() noexcept {
    if (impl_ == nullptr) return;
    impl_->nodes.Clear();
    ++impl_->generation;
}

Base::Result<void> XamlDependencyGraph::CopyDependencies(
    const Base::ResourceUri& document,
...
Base::Result<void> XamlDependencyGraph::CopyDependents(
...
Base::Result<void> XamlDependencyGraph::CollectAffected(
...
std::uint32_t XamlDependencyGraph::NodeCount() const noexcept {
    return impl_ != nullptr ? impl_->nodes.Size() : 0U;
}

std::uint64_t XamlDependencyGraph::Generation() const noexcept {
    return impl_ != nullptr ? impl_->generation : 0U;
}

struct XamlDocumentCache::Impl final {
    struct Entry final {
    ...
    Impl(
        ...
        const XamlDocumentCacheLimits& valueLimits) noexcept
        : allocator(&allocator),
          entries(&allocator),
          graph(&allocator),
          limits(valueLimits) {}
    ...
    XamlDependencyGraph graph;
    XamlDocumentCacheLimits limits;
...
XamlDocumentCache::XamlDocumentCache(
    Base::IAllocator* allocator,
...
XamlDocumentCache::~XamlDocumentCache() noexcept {
    if (impl_ == nullptr) return;
    impl_->~Impl();
    allocator_->Deallocate(
        impl_, sizeof(Impl), alignof(Impl), Base::MemoryTag::Markup);
...
XamlDocumentCache::XamlDocumentCache(
    XamlDocumentCache&& other) noexcept
    : allocator_(other.allocator_), impl_(other.impl_) {
    other.allocator_ = nullptr;
    other.impl_ = nullptr;
}

XamlDocumentCache& XamlDocumentCache::operator=(
    XamlDocumentCache&& other) noexcept {
    if (this == &other) return *this;
    if (impl_ != nullptr) {
        impl_->~Impl();
...
Base::Result<std::uint32_t> XamlDocumentCache::Invalidate(
    const Base::ResourceUri& uri,
...
void XamlDocumentCache::Clear() noexcept {
    if (impl_ == nullptr) return;
    impl_->entries.Clear();
    impl_->graph.Clear();
...
bool XamlDocumentCache::Contains(
...
Base::Result<void> XamlDocumentCache::CollectAffected(
    const Base::ResourceUri& changed,
    Base::Vector<Base::ResourceUri>& output) const noexcept {
...
const XamlDependencyGraph& XamlDocumentCache::Dependencies() const noexcept {
    static const XamlDependencyGraph empty;
    return impl_ != nullptr ? impl_->graph : empty;
}

XamlDocumentCacheStatistics XamlDocumentCache::Statistics() const noexcept {
    XamlDocumentCacheStatistics result;
...
const XamlDocumentCacheLimits& XamlDocumentCache::Limits() const noexcept {
    static const XamlDocumentCacheLimits empty{};
