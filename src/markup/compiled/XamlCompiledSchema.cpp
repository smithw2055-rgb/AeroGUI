...
Base::Result<XamlSchemaTypeInfo> ResolveTypeInfo(
...
Base::Result<XamlSchemaTypeInfo> ResolveTypeInfo(
    const XamlSchemaManifest& schema,
    Base::StringView xamlNamespace,
    Base::StringView localName) noexcept {
    ...
    for (const XamlNode& node : document.Nodes()) {
        ...
        case XamlNodeKind::StartObject: {
            const bool nullObject =
                node.Name().NamespaceUri() == XamlLanguageNamespaceUri() &&
            ...
            if (propertyElement && !frames.Empty() &&
                ...
                Base::Result<XamlResolvedMember> member = schema.ResolveMember(
                    frames.Back().type,
                    node.Name(),
            ...
            Base::Result<XamlSchemaTypeInfo> type = ResolveTypeInfo(
                schema,
...
Base::Result<XamlCompiledDocument> XamlCompiledDocument::Compile(
...
Base::Result<void> XamlCompiledDocument::ValidateSchema(
    ...
    if (CompareXamlCompiledCacheIdentity(identity_, manifest.Identity()) !=
