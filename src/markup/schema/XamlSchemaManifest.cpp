...
struct XamlSchemaManifest::Impl final {
    ...
    Base::Result<XamlResolvedMember> ResolvePropertyOrEvent(
        ...
        if (property != nullptr) {
            ...
            if (ownerWasExplicit &&
                syntax == XamlMemberSyntax::Attribute && !attached) {
                return Base::Status::Failure(
                    Base::ErrorCode::InvalidArgument,
                    "Explicit XAML attribute owner requires an attached property");
            }
            if (ownerWasExplicit &&
                syntax == XamlMemberSyntax::PropertyElement && !attached &&
...
void DestroyImpl(
    Base::IAllocator& allocator,
    XamlSchemaManifest::Impl*& impl) noexcept {
    if (impl == nullptr) return;
    impl->~Impl();
    allocator.Deallocate(
        impl,
        sizeof(XamlSchemaManifest::Impl),
        alignof(XamlSchemaManifest::Impl),
...
Base::Result<void> AppendIdentity(
    Base::Vector<std::uint8_t>& output,
    const XamlCompiledCacheIdentity& identity) noexcept {
    ...
    if (!result) return result.GetStatus();
    result = AppendU32(output, identity.descriptorFormatVersion);
    if (!result) return result.GetStatus();
    result = AppendU32(output, identity.facetFormatVersion);
    if (!result) return result.GetStatus();
    result = AppendU32(output, identity.xamlSchemaVersion);
    if (!result) return result.GetStatus();
    return AppendU64(output, identity.metadataSchemaHash);
}

Base::Result<XamlCompiledCacheIdentity> ReadIdentity(
    ...
    value = decoder.ReadU32();
    if (!value) return value.GetStatus();
    identity.descriptorFormatVersion = value.Value();
    value = decoder.ReadU32();
    if (!value) return value.GetStatus();
    identity.facetFormatVersion = value.Value();
    value = decoder.ReadU32();
    if (!value) return value.GetStatus();
    identity.xamlSchemaVersion = value.Value();
...
XamlSchemaManifest::XamlSchemaManifest(
    Base::IAllocator* allocator) noexcept
...
Base::Result<XamlSchemaManifest> XamlSchemaManifest::Capture(
    ...
    const Core::MetadataDescriptorStore& descriptors = schema.Descriptors();
    ...
    for (const Core::MetadataTypeDescriptor& type : descriptors.Types()) {
        ...
        for (Core::MemberId memberId : type.DeclaredProperties()) {
            ...
            if (property == nullptr) {
                ...
                return InvalidManifest(
                    "Metadata property descriptor is missing during manifest capture");
            }
        ...
        for (Core::MemberId memberId : type.DeclaredEvents()) {
            const Core::MetadataEventDescriptor* event =
                descriptors.FindEvent(memberId);
            if (event == nullptr) {
                DestroyImpl(selected, impl);
                return InvalidManifest(
                    "Metadata event descriptor is missing during manifest capture");
            }
            Impl::MemberRecord member(selected);
            member.id = event->Id();
    ...
    impl->valid = true;
    return XamlSchemaManifest(selected, impl);
}

Base::Result<XamlSchemaManifest> XamlSchemaManifest::Deserialize(
    ...
    Base::Result<XamlCompiledCacheIdentity> identity = ReadIdentity(decoder);
    if (!identity) {
        DestroyImpl(selected, impl);
        return identity.GetStatus();
    }
    XamlCompiledCacheIdentity current;
    current.metadataSchemaHash = identity.Value().metadataSchemaHash;
    if (CompareXamlCompiledCacheIdentity(identity.Value(), current) !=
...
XamlSchemaManifest::Serialize() const noexcept {
...
bool XamlSchemaManifest::IsValid() const noexcept {
    return impl_ != nullptr && impl_->valid;
}

std::uint32_t XamlSchemaManifest::TypeCount() const noexcept {
    return IsValid() ? impl_->types.Size() : 0U;
}

std::uint32_t XamlSchemaManifest::MemberCount() const noexcept {
    return IsValid() ? impl_->members.Size() : 0U;
}

const XamlCompiledCacheIdentity& XamlSchemaManifest::Identity() const noexcept {
    AERO_ASSERT(IsValid());
    return impl_->identity;
}

Base::Result<XamlSchemaTypeInfo> XamlSchemaManifest::ResolveType(
    Base::StringView xamlNamespace,
    ...
    return XamlSchemaTypeInfo{type->id, type->kind, type->flags};
}

Base::Result<XamlResolvedMember> XamlSchemaManifest::ResolveMember(
    Core::TypeId targetType,
...
Base::Result<XamlResolvedMember> XamlSchemaManifest::ResolveContentMember(
