...
public:
    ...
    // DataTemplate roots can be created while an ItemsSource binding is
    // being flushed. Attach cannot safely re-enter that flush, so retain the
    // root until the next bounded DataBind activation wave.
    Base::Result<void> ActivateDeferredWhenReady(
        DependencyObject& target) noexcept;
    template<
        class TSourceOwner,
        class TValue,
