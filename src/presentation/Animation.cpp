...
struct AnimationManager::Track final {
    ...
    bool pendingInitialPresentation = false;
};
...
Base::Result<void> AnimationManager::Initialize() noexcept {
    ...
    Base::Result<Core::DispatcherFrameHookHandle> hook =
        dispatcher_->RegisterFrameHook(
            ...
            &AnimationManager::AnimationFrameHook,
            this);
...
Base::Result<AnimationManager::Track*>
AnimationManager::AddTrack() noexcept {
    if (!frameHook_.IsValid()) {
        return Base::Status::Failure(
            Base::ErrorCode::NotInitialized,
            "AnimationManager is not initialized");
    }
    if (trackCount_ == trackCapacity_) {
        ...
        void* allocation = allocator_->Allocate({
            bytes, alignof(Track), Base::MemoryTag::Presentation});
...
Base::Result<AnimationHandle> AnimationManager::Begin(
    ...
    track.pendingInitialPresentation = automaticTickingEnabled_;
    return track.handle;
}
...
AnimationManager::Track* AnimationManager::FindTrack(
...
const AnimationManager::Track* AnimationManager::FindTrack(
...
Base::Result<void> AnimationManager::Pause(
    AnimationHandle handle) noexcept {
    Base::Result<void> access = dispatcher_->VerifyAccess();
    if (!access) return access.GetStatus();
...
Base::Result<void> AnimationManager::Resume(
    AnimationHandle handle) noexcept {
    Base::Result<void> access = dispatcher_->VerifyAccess();
    if (!access) return access.GetStatus();
    Track* track = FindTrack(handle);
...
Base::Result<void> AnimationManager::Seek(
    AnimationHandle handle,
    ...
    track->startTimeMicroseconds = currentTimeMicroseconds_;
    track->pendingInitialPresentation = false;
...
Base::Result<void> AnimationManager::ClearTrackValue(
...
Base::Result<void> AnimationManager::Stop(
    AnimationHandle handle) noexcept {
    Base::Result<void> access = dispatcher_->VerifyAccess();
    if (!access) return access.GetStatus();
    Track* track = FindTrack(handle);
...
Base::Result<void> AnimationManager::Remove(
    AnimationHandle handle) noexcept {
...
Base::Result<std::uint32_t> AnimationManager::RemoveTarget(
...
Base::Result<void> AnimationManager::RemoveAll() noexcept {
    if (dispatcher_ == nullptr || !dispatcher_->CheckAccess()) {
        return Base::Status::Failure(
            Base::ErrorCode::WrongThread,
            "AnimationManager removal requires dispatcher access");
    }
    bool changed = false;
    for (std::uint32_t index = 0U; index < trackCount_; ++index) {
...
Base::Result<bool> AnimationManager::ApplyTrack(
    Track& track,
    ...
    AnimationTime sampledNow = track.pendingInitialPresentation
        ? track.startTimeMicroseconds
        : nowMicroseconds;
    if (!track.pendingInitialPresentation &&
        track.state == AnimationState::Paused) {
...
Base::Result<std::uint32_t> AnimationManager::Tick(
    AnimationTime nowMicroseconds) noexcept {
    Base::Result<void> access = dispatcher_->VerifyAccess();
    if (!access) return access.GetStatus();
    if (!frameHook_.IsValid()) {
    ...
    if (ticking_) {
        return Base::Status::Failure(
            ...
            "AnimationManager tick is already active");
    }
...
AnimationManager::ApplyPendingInitialValues() noexcept {
    if (!automaticTickingEnabled_) {
        return 0U;
    }
    bool hasPending = false;
    for (std::uint32_t index = 0U; index < trackCount_; ++index) {
        if (tracks_[index].pendingInitialPresentation) {
            hasPending = true;
            break;
        }
    }
...
void AnimationManager::CommitPendingInitialValues() noexcept {
...
Base::Result<std::uint32_t> AnimationManager::AdvanceBy(
...
AnimationDiagnostics AnimationManager::Diagnostics() const noexcept {
...
void AnimationManager::CompactStopped() noexcept {
...
void AnimationManager::ReleaseTracks() noexcept {
    for (std::uint32_t index = 0U; index < trackCount_; ++index) {
        tracks_[index].~Track();
    }
    if (tracks_ != nullptr) {
...
void AnimationManager::AnimationFrameHook(void* context) noexcept {
    auto* manager = static_cast<AnimationManager*>(context);
