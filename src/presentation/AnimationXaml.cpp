...
Base::Result<Presentation::AnimationTime> ParseClockTime(
    ...
    return static_cast<Presentation::AnimationTime>(
...
Base::Result<void> Timeline::SetBeginTime(
    ...
    Base::Result<Presentation::AnimationTime> parsed =
...
Base::Result<void> Timeline::SetRepeatBehavior(
    ...
    if (Core::ValueConversion::EqualsAsciiInsensitive(
            trimmed, "Forever")) {
        repeat = Presentation::RepeatBehavior::Forever();
    } else {
        ...
        repeat = Presentation::RepeatBehavior::Count(count.Value());
    }
...
Presentation::DoubleAnimation
DoubleAnimation::RuntimeAnimation() const noexcept {
    Presentation::DoubleAnimation result;
    result.from = from_;
    result.to = to_;
...
Presentation::ColorAnimation
ColorAnimation::RuntimeAnimation() const noexcept {
    Presentation::ColorAnimation result;
    result.from = from_;
    result.to = to_;
    result.timing = Timing();
    if (easing_) result.easing = easing_->RuntimeEasing();
...
PointAnimation::RuntimeAnimation() const noexcept {
    Presentation::PointAnimation result;
    result.from = from_;
    result.to = to_;
    result.timing = Timing();
    if (easing_) {
...
Presentation::RectAnimation
RectAnimation::RuntimeAnimation() const noexcept {
    Presentation::RectAnimation result;
    result.from = from_;
    result.to = to_;
    result.timing = Timing();
    if (easing_) {
...
Presentation::ThicknessAnimation
ThicknessAnimation::RuntimeAnimation() const noexcept {
    Presentation::ThicknessAnimation result;
    result.from = from_;
    result.to = to_;
    result.timing = Timing();
    if (easing_) {
...
Base::Result<void> SplineDoubleKeyFrame::SetKeySpline(
    ...
    if (!assigned) return assigned.GetStatus();
    Presentation::DoubleKeyFrame& frame = MutableFrame();
...
SplineColorKeyFrame::SetKeySpline(
    ...
    assigned = keySpline_.TryAssign(value);
    if (!assigned) return assigned.GetStatus();
    Presentation::ColorKeyFrame& frame =
        MutableFrame();
    frame.controlPoint1X = values[0];
    frame.controlPoint1Y = values[1];
    frame.controlPoint2X = values[2];
