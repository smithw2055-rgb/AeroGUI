...
RoutedEventCatalog::RoutedEventCatalog(
    TypeRegistry& types,
...
Base::Result<RoutedEventHandle> RoutedEventCatalog::TryRegister(
...
Base::Result<void> RoutedEventCatalog::Freeze() noexcept {
    if (frozen_) return {};
    if (!types_->IsFrozen()) {
        return InvalidState(
...
const RoutedEventCatalog::Definition* RoutedEventCatalog::Find(
