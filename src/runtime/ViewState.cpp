...
#include "controls/ControlBehaviorService.hpp"
...
struct ViewState::Impl final {
    ...
    Controls::Detail::ControlBehaviorService* controlBehaviors = nullptr;
    ...
    MeshServices() noexcept {
        return endpoint
            ? Integration::Detail::RenderEndpointAccess::MeshServices(*endpoint)
            : nullptr;
    }
    ...
    ImageServices() noexcept {
        return endpoint
            ? Integration::Detail::RenderEndpointAccess::ImageServices(*endpoint)
            : nullptr;
    }
            ...
            AttachTextService(
                *node,
                textRuntime != nullptr
                    ? textRuntime->Service()
                    : nullptr);
            AttachPathService(*node, MeshServices());
            if (metadata->Types().IsDerivedFrom(
                    type,
    ...
    if (impl_->textRuntime != nullptr) {
        Base::Result<bool> synchronized =
            impl_->textRuntime->SynchronizeBackend(
                *impl_->endpoint, endpointGenerationChanged);
        if (!synchronized) {
    ...
    for (Core::DispatcherFramePhase phase : phases) {
        ...
        const std::uint64_t renderVersionBefore =
            phase == Core::DispatcherFramePhase::RenderCommit
                ? impl_->renderer->CurrentFrame().Version() : 0U;
        ...
        if (phase ==
            ...
            const Render::RenderFrame& frame = impl_->renderer->CurrentFrame();
            if (impl_->endpoint && frame.Version() != renderVersionBefore) {
                Base::Result<void> submitted =
                    Integration::Detail::RenderEndpointAccess::Submit(*impl_->endpoint, frame);
...
ViewState::DispatchPointer(
    ...
    Base::Result<
        ...
        dispatched =
            impl_->input->DispatchPointer(input);
    if (!dispatched) {
...
ViewState::DispatchKeyboard(
    ...
    return impl_->input->DispatchKeyboard(input);
}
...
ViewState::AdvanceTime(
    ...
    std::uint32_t actionCount = 0U;
    if (impl_->controlBehaviors != nullptr) {
        Base::Result<std::uint32_t> controls =
            impl_->controlBehaviors->AdvanceTime(
                elapsedMilliseconds);
...
Base::Result<void> ViewState::SetRenderEndpoint(
    ...
    if (impl_->textRuntime != nullptr) {
        Base::Result<bool> synchronized =
            impl_->textRuntime->SynchronizeBackend(*impl_->endpoint, true);
        if (!synchronized) {
