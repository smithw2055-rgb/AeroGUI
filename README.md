...
- 已完成 OpenGL 3.3 基础合同、RHI 与 Windows/WGL 实现切片：host-injected function table、3.3 Core Profile/当前线程/context generation 验证、capability/limits 查询、完整 state cache，以及 buffer、texture、sampler、GLSL 330 pipeline、render pass、bind/draw、GLsync、readback 和外部导入；WGL adapter 支持 owned/borrowed context、pixel format、swap interval、resize、present 和 context recreation，并由 hidden-window 真 Core 3.3 绘制/SwapBuffers conformance 覆盖。
- 尚未完成：完整 Unicode line breaking/bidi、OpenGL RenderPlan lowering、Linux GLX、D3D11/GL 共享像素 conformance、TextBox/IME、ControlGallery 与最终性能/稳健性门禁。TextBlock 渲染服务已支持在 loss 后放弃旧 handles、重绑定宿主重建的 device/backend，并由下一次布局重建 atlas 与 glyph runs。
