...
- 已完成 OpenGL 3.3 基础合同切片：host-injected function table、3.3 Core Profile/当前线程/context generation 验证、capability/limits 查询，以及覆盖 program/VAO/buffer/framebuffer/viewport/scissor/blend/depth/stencil/texture/sampler/pixel-unpack 的 state cache；支持 `HostReset` 与 `PreserveAndRestore` 两种嵌入模式。
- 尚未完成：完整 Unicode line breaking/bidi、实际 OpenGL 3.3 RHI 与 RenderPlan lowering、WGL/GLX、D3D11/GL 共享像素 conformance、TextBox/IME、ControlGallery 与最终性能/稳健性门禁。TextBlock 渲染服务已支持在 loss 后放弃旧 handles、重绑定宿主重建的 device/backend，并由下一次布局重建 atlas 与 glyph runs。
