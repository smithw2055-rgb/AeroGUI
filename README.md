...
- 已建立 `AeroText` 的 provider-neutral 合同层，并完成可独立裁剪的 FreeType provider、HarfBuzz shaper、provider-neutral glyph atlas 与 `TextLayout::ShapeAndMeasure` 基础排版；固定字体测试覆盖 Latin、数字、中文、Arabic、稳定测量、word/character wrapping、ellipsis trimming、水平对齐、行高、glyph metrics、Gray8 raster、outline、DPI、face cache/lifetime、atlas page/shelf、fence-safe reuse 和 device-loss generation。
- 尚未完成：fallback 分段、完整 Unicode line breaking/bidi、atlas GPU 上传/注册与 TextBlock 自动排版，以及 Button/Toggle/Scroll/Items/ListBox、recycling virtualization、OpenGL 3.3/WGL/GLX、TextBox/IME、ControlGallery 与对应性能/稳健性门禁。
