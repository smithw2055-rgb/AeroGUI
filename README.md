...
- 尚未完成：完整 Unicode line breaking/bidi、IME、ControlGallery 与最终性能/稳健性门禁。TextBlock 渲染服务已支持在 loss 后放弃旧 handles、重绑定宿主重建的 device/backend，并由下一次布局重建 atlas 与 glyph runs。
