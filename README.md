...
- 已完成的 M3 基础：Binding/DataContext、通知驱动更新、Style/ControlTemplate/TemplateBinding/property trigger、compiled XAML document、module SDK、`aero-schema-gen`、manifest-driven `aero-xamlc`、共享 XAML document cache、URI 依赖图和完整文档热重载协调器；0.3 SDK 已提供 `Aero::EngineHost` / `Aero::ModuleSdk` 产品入口、安装后消费门禁和标准 ControlGallery dogfood 流程。
