...
- 已完成 OpenGL 3.3 基础合同与 RHI 实现切片：host-injected function table、3.3 Core Profile/当前线程/context generation 验证、capability/limits 查询、覆盖 program/VAO/buffer/read+draw framebuffer/viewport/scissor/blend/depth/stencil/texture/sampler/pixel-unpack 的 state cache，以及 buffer、2D/array/multisample texture、sampler、GLSL 330 pipeline、render pass、bind/draw、GLsync fence/延迟删除、上下翻转 readback 和宿主外部 texture/render-target 导入；支持 `HostReset` 与 `PreserveAndRestore` 两种嵌入模式，并由注入式 fake-GL conformance 覆盖资源/提交/读回/上下文错误路径。
- 尚未完成：完整 Unicode line breaking/bidi、OpenGL RenderPlan lowering、WGL/GLX 真实 context/surface、D3D11/GL 共享像素 conformance、TextBox/IME、ControlGallery 与最终性能/稳健性门禁。TextBlock 渲染服务已支持在 loss 后放弃旧 handles、重绑定宿主重建的 device/backend，并由下一次布局重建 atlas 与 glyph runs。
