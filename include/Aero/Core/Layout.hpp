...
AERO_NODISCARD AERO_API double RoundLayoutValue(
    double value,
...
public:
    ...
    AERO_NODISCARD std::uint64_t LayoutRevision() const noexcept {
    ...
    AERO_NODISCARD Base::Result<void> SetLayoutRounding(
        bool enabled,
        double dpiScale = 1.0) noexcept;

protected:
    AERO_NODISCARD virtual Base::Result<Size> MeasureOverride(
        Size availableSize) noexcept;
    AERO_NODISCARD virtual Base::Result<Size> ArrangeOverride(
...
public:
    ...
    AERO_NODISCARD Base::Result<void> Detach(
        LayoutElement& parent,
        LayoutElement& child) noexcept;
    AERO_NODISCARD Base::Result<void> SetRoot(
        LayoutElement* root,
        Size availableSize) noexcept;

    AERO_NODISCARD Base::Result<void> InvalidateMeasure(
        LayoutElement& element) noexcept;
    AERO_NODISCARD Base::Result<void> InvalidateArrange(
        LayoutElement& element) noexcept;
    AERO_NODISCARD Base::Result<std::uint32_t> Flush() noexcept;

    AERO_NODISCARD LayoutDiagnostics Diagnostics() const noexcept;
    AERO_NODISCARD std::uint64_t PassVersion() const noexcept {
        return passVersion_;
    }
...
private:
    ...
    AERO_NODISCARD Base::Result<void> QueueMeasure(
        LayoutElement& element) noexcept;
    AERO_NODISCARD Base::Result<void> QueueArrange(
        LayoutElement& element) noexcept;
    AERO_NODISCARD Base::Result<void> MeasureElement(
        LayoutElement& element,
        Size constraint) noexcept;
    AERO_NODISCARD Base::Result<void> ArrangeElement(
        LayoutElement& element,
        Rect slot) noexcept;
    void RemoveChild(
