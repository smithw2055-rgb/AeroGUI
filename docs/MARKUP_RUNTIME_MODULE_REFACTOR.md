...
6. `XamlLoadResult` contains `XamlVisualContentPlan`. Markup records content edges during loading; `View` then asks `Presentation::VisualTreeMount` to attach, resize, and detach that plan.
...
25. Runtime Style/Template application policy lives in `RuntimePresentationServices`, not in `View::Impl`.
