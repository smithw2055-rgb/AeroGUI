...
Result<Ref<Object>> UnsupportedFactory() noexcept {
    return Status::Failure(
        ErrorCode::Unsupported,
        "Test factory is not executable");
}
...
bool TestBehaviorRegistrationBoundaries() {
    ...
    CHECK(registration.TryRegisterType(TypeRegistration::Object("urn:behavior", "Object", InvalidTypeId, TypeFlags::None, &UnsupportedFactory)));
    CHECK(first.Freeze());
    CHECK(firstBehaviors.Freeze());
    return true;
}
