...
struct Fixture final {
    ...
    RoutedEventCatalog eventCatalog{types, typesBehaviors};
    ...
    bool Build() {
        ...
        Result<RoutedEventHandle> bubbleResult = eventCatalog.TryRegister({
            StringView("Click"), elementType, eventArgsType,
            RoutingStrategy::Bubble});
        CHECK(bubbleResult);
        bubble = bubbleResult.Value();

        Result<RoutedEventHandle> tunnelResult = eventCatalog.TryRegister({
            StringView("PreviewClick"), elementType, eventArgsType,
            RoutingStrategy::Tunnel});
        CHECK(tunnelResult);
        tunnel = tunnelResult.Value();

        Result<RoutedEventHandle> directResult = eventCatalog.TryRegister({
...
bool TestClassHandlersAndRegistrationRules() {
    ...
    Result<RoutedEventHandle> late = fixture.eventCatalog.TryRegister({
